...
| A3 | **Grab whole pile** (modifier-drag or long-press on a stack/deck) | RMB-drag / container drag | **M** | *Shipped for decks; gesture revised by tableplace-103.* #91's move-on-any-travel had the frequencies backwards, so the pile move now costs a deliberate **long press** (400ms, `DECK_MOVE_HOLD_MS`): the deck lifts and wears an amber outline while armed, then travel drags the whole deck (streams through the position throttle like cards, Esc returns it to origin). Plain travel instead **draws the top card into the in-flight drag** (`drawIntoDrag` + `dragStart(cardId)` at threshold-crossing), tap still draws 1 to the felt, and an armed press released without travel does nothing. Loose stacks still wait on the selection model. |
| A4 | **Split / take N** (drag off top with count, or cut at midpoint) | number+drag, Cut | **M** | Take-1-by-drag shipped with tableplace-103 (see A3); take-N / cut still open. |
