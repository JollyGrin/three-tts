<script lang="ts">
	...
	let rot = $state(0);
	let scale = $state($gameStore?.overlays?.unmatched?.scale ?? 0.015 * 100);
	let point3d = $state({ x: 0, y: 0 });
	let imageUrl = $state($gameStore?.overlays?.unmatched?.imageUrl ?? '');

	// $effect(() => {
	// 	const _imageUrl = imageUrl === '' ? undefined : imageUrl;
	// 	gameStore.updateState({
	// 		overlays: {
	// 			unmatched: purgeUndefinedValues({
	// 				imageUrl: _imageUrl,
...
<Pane
	...
	y={0}
	x={0}
	localStoreId="unmatched-settings"
>
	<FpsGraph />
	<Folder title="Connection" expanded={false}>
		...
		<Button
			...
			on:click={() => {
				goto(`/unmatched?${lobbyUrl}`, { invalidateAll: true });
				const hostUrl = page.url.host ?? '';
				const url = `${hostUrl}/unmatched?${lobbyUrl}`;
